# 1. Basic run (default filename): python "stage 5.py" https://raw.githubusercontent.com/python/cpython/main/README.rst
# 2. Custom filename: python "stage 5.py" https://raw.githubusercontent.com/python/cpython/main/README.rst -o python_readme.rst
import os
import time
import argparse
import requests
...
def download_file(url, output_filename=None, max_redirects=5):
    session = requests.Session()
    session.max_redirects = max_redirects

    try:
        # Send request with redirect allowed
        with session.get(url, stream=True, allow_redirects=True, timeout=10) as res:
            res.raise_for_status()

            # Use final redirected URL to get filename
            final_url = res.url
            ...
            # Get total size and initialize download stats
            ...
            # stream download with progress/speed/ETA
        ...
        print(f"\nError: Too many redirects (max allowed: {max_redirects})")
        print(f"URL: {url}")
    except Timeout:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="wget tool (Stage 5)")
